...
from collision.util import flatten_points_on
import pygame as pg
...
class Ant():
    ...
    def update(self):
        ...
        if self.food_target != None: 
            if col.collide(self.rect,self.food_target.rect):
                self.mission = False
        ...
        for food in food_list:
            if not self.food_found:
                if col.collide(food.rect,self.vision):
                    self.mission  = True
                ...
                if not self.food_found:
                    if self.want_to_go == 'food':
                        if phero.type == 'food':
                            ...
                            if self.phero_target in phero_list:
...
class Phero():
    ...
    def update(self):
        ...
        if self.type == 'home':
            div = self.green/PHERO_LENGTH*2
            self.green -= div
        elif self.type == 'food':
            div = self.red/PHERO_LENGTH*2
            self.red -= div
        self.tick += 1  
...
while go:
    ...
    if start:
        ...
        for ant in ant_list:
            if ant.phero_space >= randint(0,PHERO_PERIOD):
                ant_pos = ant.pos
                if ant.food_found:
